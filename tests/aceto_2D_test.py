...
for mol in molecules:
    rwa += mol.elenergies[1]
rwa = rwa/len(molecules)
...
t2s = TimeAxis(0.0, 50, 10.0)

tcalc = TwoDResponseCalculator(t1axis=ta, t2axis=t2s, t3axis=ta,
                               system=agg)
...
w1_min = rwa_cm - 1000.0
w1_max = rwa_cm + 1000.0
w3_min = rwa_cm - 1000.0
w3_max = rwa_cm + 1000.0
...
#    cbmax = twods[mloc].plot(axis=[w1_min, w1_max, w3_min, w3_max],vmax=1.0)
    ...
    for tt2 in t2s.data:
        ...
        sp.get_TwoDSpectrum(dtype=qr.signal_TOTL)
        sp.plot(window=[w1_min, w1_max, w3_min, w3_max]) #,vmax=1.0, cbmax=cbmax)
